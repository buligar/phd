"""
SPC vs EDC comparison for two reference dynamics:
...
Architectures:
- EDC: latent state z is driven directly by continuous error e = o1 - g.
- SPC: error e is encoded by a separate spiking error population, decoded as e_dec,
...
- overview figures:
    ...
    row 2: error, spikes o1, spikes error population only for SPC, spikes latent z/o2
- component-signal figures: reference vs autonomous o1 vs g;
- summary CSV and comparison figures.

Important:
The error spiking raster is shown only for SPC, because only SPC has a real
spiking error population. EDC uses a continuous error signal, so no
...
class Config:
    ...
    # Plotting
    out_dir: str = "results_SPC_neSPC_lorenz_oscillator"
    dpi: int = 220
...
def plot_overview(result, cfg: Config, out_path):
    ...
    if arch == "SPC":
        ...
        # EDC has no spiking error population, so this figure must not
...
def run_architecture(
    ...
    if use_error_population:
        # SPC: this is the real spiking error population used for e_dec and z update.
        ...
        # EDC: no spiking error population is created.
    ...
    for n in range(1, N_t):
        ...
        if use_error_population:
            # SPC: the explicit spiking error population is decoded as e_dec and drives z.
            ...
            # EDC: direct continuous error, no error spikes and no monitor population.
            e_drive = e
...
def run_sweep(cfg: Config):
    ...
    print("\n=== SPC vs EDC sweep: Lorenz + oscillator ===")
