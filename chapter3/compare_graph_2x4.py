...
XLSX_PATH = "results_SPC_EDC_10tests_8metrics_4x2/summary_metrics_mean_std.xlsx"   # поменяй на свой файл
SHEET_NAME = 0
...
def get_plot_metrics(df):
    ...
    metrics = [
        ("sim_time_s", "Время моделирования (сек)", False),
        ("sim_python_peak_mem_mb", "Пиковое потребление памяти \n Python (мб)", False),

        ("mean_amp_corr", "Корреляция амплитуды", True),
        ("mean_freq_corr", "Корреляция частоты", True),

        ("mean_plv", "Показатель фазовой синхронизации", True),
    ]

    if "mean_phase_deg" in df.columns or "mean_phase_deg_mean" in df.columns:
        metrics.append(("mean_phase_deg", "Средняя разность фаз", False))
    elif USE_SYNC_SCORE_IF_NO_PHASE:
        metrics.append(("sync_score", "Показатель синхронизации", True))
    else:
        metrics.append(("mean_phase_deg", "Средняя разность фаз", False))

    # last row
    metrics.extend([
        ("rmse_ref_o1", r"RMSE(эталон, $o_1$)", False),
...
def plot_ijbc_2x4(df_ms, metrics, out_dir):
    ...
    for signal_name in sorted(df_ms["signal"].dropna().unique()):
        ...
        out_png = out_dir / f"ijbc_metrics_2x4_{signal_name}.png"
        out_pdf = out_dir / f"ijbc_metrics_2x4_{signal_name}.pdf"
        out_svg = out_dir / f"ijbc_metrics_2x4_{signal_name}.svg"
...
def main():
    ...
    plot_ijbc_2x4(df_ms, metrics, OUT_DIR)
